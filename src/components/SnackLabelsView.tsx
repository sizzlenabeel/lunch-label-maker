...
import { AlertTriangle, Leaf, Cookie } from 'lucide-react';
...
export function SnackLabelsView() {
  ...
  useEffect(() => {
    async function fetchSnackProducts() {
      try {
        setLoading(true);
        ...
        console.error('Error fetching snack products:', err);
      } finally {
        setLoading(false);
      }
    }
    ...
    <div className="bg-white shadow rounded-lg p-6">
      ...
      {selectedProduct ? (
        <div>
          ...
          <LabelSlotPicker slots={slots} onChange={setSlots} accent="amber" />
          <SnackLabelPDF data={convertToLabelData(selectedProduct)} slots={slots} />
        </div>
      ) : loading ? (
        <div className="text-center py-4">
          <div className="animate-pulse text-gray-500">Loading snack products...</div>
        </div>
      ) : error ? (
        ...
        <div className="text-center py-4 text-gray-500">
          No snack products found for week {selectedWeek}
        </div>
      ) : (
